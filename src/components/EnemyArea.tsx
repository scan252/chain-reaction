...
export function EnemyArea() {
  ...
  return (
    <div className="flex flex-col items-center gap-1 py-1 w-full px-4">
      {/* 血量条 */}
      <div className={`${isBoss ? 'max-w-[760px]' : 'max-w-[400px]'} w-full flex items-center gap-2 mb-1`}>
        <div className="flex-1 h-3 rounded-full bg-gray-800 border border-gray-700 overflow-hidden relative">
          <motion.div
        ...
        <span className="num text-[13px] text-red-400 font-bold whitespace-nowrap text-shadow">
          {enemy.currentHp} / {enemy.maxHp}
          {enemy.armor > 0 && (
            <span className="text-[#8fb8e0] ml-2 text-shadow">🛡 {enemy.armor}</span>
          )}
        </span>
      </div>
      ...
      {/* 敌人图像（精英/Boss 皇冠标识） */}
      <motion.div
        className={`${isMobile ? 'w-16 h-16 rounded-xl text-3xl' : 'w-24 h-24 rounded-2xl text-5xl'} bg-gradient-to-br from-red-900/50 to-red-950/80 flex items-center justify-center overflow-hidden border-2 ${
          isBoss
            ? 'border-red-500/60'
            : isElite
              ? 'border-amber-500/50'
      ...
      {/* 敌人名称 + 头衔 */}
      <motion.h2
        className={`font-bold text-red-400 tracking-wider mt-0.5 text-shadow-heavy ${isMobile ? 'text-sm' : 'text-base'}`}
        animate={isAttacking ? { scale: [1, 1.1, 1] } : {}}
        transition={{ duration: 0.5 }}
      >
        {isBoss && '👑 '}{isElite && '🎖️ '}{enemy.name}{(isBoss || isElite) && (isBoss ? ' · BOSS' : ' · 精英')}
      </motion.h2>
      ...
      {phase !== 'VICTORY' && phase !== 'DEFEAT' && (
        ...
        >
          <div className={`text-[13px] sm:text-sm flex items-center gap-1.5 px-3 py-0.5 rounded-lg border text-shadow ${getIntentTone(enemy.intent.pattern)}`}>
